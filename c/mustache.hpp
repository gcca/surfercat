...
std::vector<string_type> split(const string_type &s,
...
public:
  using type1 = std::function<string_type(const string_type &)>;
  using type2 = std::function<string_type(
      const string_type &, const basic_renderer<string_type> &render)>;
  ...
  string_type operator()(const string_type &text,
                         const basic_renderer<string_type> &render) const {
...
public:
  ...
  virtual const basic_data<string_type> *
  get_partial(const string_type &name) const = 0;
};
...
public:
  ...
  virtual const basic_data<string_type> *
  get(const string_type &name) const override {
    // process {{.}} name
  ...
  virtual const basic_data<string_type> *
  get_partial(const string_type &name) const override {
    for (const auto &item : items_) {
      const auto var = item->get(name);
      if (var) {
        return var;
...
public:
  ...
  bool is_section_begin() const {
    return type == tag_type::section_begin ||
           type == tag_type::section_begin_inverted;
  }
...
public:
  context_pusher(context_internal<string_type> &ctx,
                 const basic_data<string_type> *data)
      : ctx_(ctx) {
    ctx.ctx.push(data);
  }
...
public:
  parser(const string_type &input, context_internal<string_type> &ctx,
         component<string_type> &root_component, string_type &error_message) {
...
private:
  void parse(const string_type &input, context_internal<string_type> &ctx,
    ...
    using string_size_type = typename string_type::size_type;
    using streamstring =
        std::basic_ostringstream<typename string_type::value_type>;
    ...
    const auto process_current_text = [&current_text, &current_text_position,
                                       &sections]() {
    ...
    for (string_size_type input_position = 0; input_position != input_size;) {
      ...
      if (input.compare(input_position, ctx.delim_set.begin.size(),
                        ctx.delim_set.begin) == 0) {
        ...
        for (const auto &whitespace_text : whitespace) {
          if (input.compare(input_position, whitespace_text.size(),
                            whitespace_text) == 0) {
            process_current_text();
      ...
      string_size_type tag_contents_location{tag_location_start +
                                             ctx.delim_set.begin.size()};
      const bool tag_is_unescaped_var{current_delimiter_is_brace &&
                                      tag_location_start != (input_size - 2) &&
                                      input.at(tag_contents_location) ==
                                          ctx.delim_set.begin.at(0)};
      const string_type &current_tag_delimiter_end{
          tag_is_unescaped_var ? brace_delimiter_end_unescaped
                               : ctx.delim_set.end};
      const auto current_tag_delimiter_end_size =
          current_tag_delimiter_end.size();
      ...
      const string_type tag_contents{
          trim(string_type{input, tag_contents_location,
      ...
      if (comp.tag.is_section_begin()) {
        ...
        if (sections.size() == 1) {
          streamstring ss;
          ss << "Unopened section \"" << comp.tag.name << "\" at "
             << comp.position;
          error_message.assign(ss.str());
          return;
        }
        sections.back()->tag.section_text.reset(new string_type(
            input.substr(section_starts.back(),
    ...
    root_component.walk_children(
        ...
        typename component<string_type>::walk_control {
          ...
          if (comp.children.empty() ||
              !comp.children.back().tag.is_section_end() ||
              comp.children.back().tag.name != comp.tag.name) {
            streamstring ss;
            ss << "Unclosed section \"" << comp.tag.name << "\" at "
               << comp.position;
            error_message.assign(ss.str());
  ...
  bool
  parse_set_delimiter_tag(const string_type &contents,
    ...
    const string_type end =
        contents_substr.substr(nonspace, contents_substr.size() - nonspace);
  ...
  void parse_tag_contents(bool is_unescaped_var, const string_type &contents,
...
public:
  ...
  stream_type &render(const basic_data<string_type> &data,
                      stream_type &stream) {
  ...
  void render(const basic_data<string_type> &data,
...
private:
  ...
  void render_result(context_internal<string_type> &ctx,
  ...
  render_component(const render_handler &handler,
    ...
    case tag_type::unescaped_variable:
      if ((var = ctx.ctx.get(tag.name)) != nullptr) {
        if (!render_variable(handler, var, ctx,
                             tag.type == tag_type::variable)) {
          return component<string_type>::walk_control::stop;
        }
      }
    ...
    case tag_type::section_begin:
      if ((var = ctx.ctx.get(tag.name)) != nullptr) {
        if (var->is_lambda() || var->is_lambda2()) {
          if (!render_lambda(handler, var, ctx, render_lambda_escape::optional,
                             *comp.tag.section_text, true)) {
            return component<string_type>::walk_control::stop;
          }
  ...
  bool render_lambda(const render_handler &handler,
                     ...
                     context_internal<string_type> &ctx,
                     render_lambda_escape escape, const string_type &text,
                     bool parse_with_same_context) {
    const typename basic_renderer<string_type>::type2 render2 =
        [this, &ctx, parse_with_same_context, escape](const string_type &text,
                                                      bool escaped) {
          const auto process_template =
              [this, &ctx, escape,
               escaped](basic_mustache &tmpl) -> string_type {
            if (!tmpl.is_valid()) {
              error_message_ = tmpl.error_message();
              return {};
            }
            context_internal<string_type> render_ctx{
                ctx.ctx}; // start a new line_buffer
  ...
  bool render_variable(const render_handler &handler,
    ...
    if (var->is_string()) {
      ...
      const render_lambda_escape escape_opt =
          escaped ? render_lambda_escape::escape
                  : render_lambda_escape::unescape;
  ...
  void render_section(const render_handler &handler,
                      ...
                      const basic_data<string_type> *var) {
    const auto callback = [&handler, &ctx,
                           this](component<string_type> &comp) ->
        typename component<string_type>::walk_control {
          return render_component(handler, ctx, comp);
        };
